...
export default function CourtsPage() {
  ...
  const handleAddCourt = async () => {
    ...
    const exists = courts.some((c: Court) => c.name.toLowerCase() === formattedName.toLowerCase());
  ...
  const handleScoreSubmit = async (teamAScore: number | undefined, teamBScore: number | undefined, winner: 'teamA' | 'teamB') => {
    if (scoringCourtId) {
      await endMatch(scoringCourtId, 'completed', winner, teamAScore, teamBScore);
  ...
  return (
    <div className="container mx-auto px-4 py-4 md:py-8 space-y-6 md:space-y-8 pb-24 max-w-7xl h-full overflow-auto">
      <div className="flex items-center justify-between shrink-0">
        <h1 className="text-xl md:text-2xl font-bold">Courts</h1>
        <div className="flex gap-2">
          <Button onClick={handleGenerateMatch} disabled={loadingMatch || !courts.length} className="gap-2 bg-primary transition-all active:scale-95 shadow-md shadow-primary/10 text-sm md:text-base">
      ...
      {scoringCourtId && (() => {
        const court = courts.find((c: Court) => c.id === scoringCourtId);
        const match = matches.find((m: Match) => m.id === court?.currentMatchId);
        if (!match) return null;

        const teamA = players.filter((p: Player) => match.teamA.includes(p.id));
        const teamB = players.filter((p: Player) => match.teamB.includes(p.id));
