...
import { Trophy, TrendingUp, Medal, Star, Target, Calendar, FileDown } from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs';
import { cn } from '@/lib/utils';
import { exportRankingsToCSV } from '@/lib/export';
...
export default function SessionRankingsPage() {
  ...
  const { players, matches, currentSession, selectSession } = useClub();

  const sessionIdStr = Array.isArray(sessionId) ? sessionId[0] : sessionId;

  useEffect(() => {
    if (sessionIdStr) {
      selectSession(sessionIdStr);
      localStorage.setItem('tbc_current_session_id', sessionIdStr);
    }
  }, [sessionIdStr, selectSession]);

  // Filter matches for this session only
  const sessionMatches = useMemo(() => {
    return matches.filter(m => m.sessionId === sessionIdStr);
  }, [matches, sessionIdStr]);
  ...
  const todayMatches = useMemo(() => {
    ...
    return sessionMatches.filter(m => new Date(m.timestamp).toDateString() === now);
  }, [sessionMatches]);

  const monthMatches = useMemo(() => {
    const now = new Date();
    return sessionMatches.filter(m => {
  ...
  return (
    <div className="container mx-auto px-4 py-4 md:py-6 space-y-4 md:space-y-6 max-w-5xl h-full overflow-auto">
      <header className="space-y-0.5 text-center sm:text-left shrink-0 flex items-start justify-between">
        <div className="space-y-0.5">
          <h1 className="flex items-center justify-center sm:justify-start gap-2 md:gap-3">
            <Trophy className="h-6 w-6 md:h-8 md:w-8 text-primary" /> Session Leaderboards
          </h1>
          <p className="text-tiny text-muted-foreground font-black uppercase tracking-widest opacity-60">Stats for this session only</p>
        </div>
        <Button
          variant="outline"
          size="sm"
